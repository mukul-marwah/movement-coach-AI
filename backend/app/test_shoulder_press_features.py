from analysis.features import extract_dumbbell_shoulder_press_features

landmarks = []
for _ in range(14):
    landmarks.append({"x": 0.0, "y": 0.0, "z": 0.0})

movement_data = [{"frame": 0, "timestamp_ms": 0, "world_landmarks": landmarks}]
features = extract_dumbbell_shoulder_press_features(movement_data)

expected_features = {
    "left_elbow_angle",
    "right_elbow_angle",
    "left_shoulder_angle",
    "right_shoulder_angle",
}

assert len(features) == 1
assert expected_features.issubset(features[0].keys())

print("Dumbbell shoulder press feature test: PASS")
print("Features generated:", len(features[0]))

for name in expected_features:
    print(f"{name}: {features[0][name]}")